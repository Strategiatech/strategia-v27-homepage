...
export function initTetrahedron(): () => void {
  ...
  const _hero = document.querySelector('.v15-hero') as HTMLElement | null
  const _stage = document.getElementById('v15Stage') as HTMLElement | null
  const _edgesG = document.getElementById('v15Edges')
  const _glowEl = document.getElementById('v15Glow')
  const _stop0 = document.getElementById('v15GlowStop0')
  const _stop1 = document.getElementById('v15GlowStop1')
  const cursorAura = document.getElementById('v15CursorAura')
  const plasmaLayer = document.getElementById('v15PlasmaLayer')
  const _pulsesG = document.getElementById('v15Pulses')
  const pulseOverlay = document.getElementById('v15PulseOverlay')
  const spacer = document.querySelector('.v15-scroll-spacer') as HTMLElement | null
  ...
  const GLOW_BASE_R = 55
  const GLOW_MAX_R = 95
  const GLOW_BASE_A = 0.45
  const GLOW_MAX_A = 0.95
  ...
  function completeInteraction() {
    ...
    targetGlow = 1
    window.scrollTo(0, 0)
    window.dispatchEvent(new CustomEvent('v15:complete'))
  }
  ...
  function spawnPlasmaBlob() {
    ...
    blob.className = 'v15-plasma-blob'
  ...
  const wireFrontRGB = { r: 6, g: 41, b: 62 }
  const wireBackRGB = { r: 107, g: 115, b: 160 }
  ...
  function spawnPulse() {
    ...
    ring.setAttribute('fill', 'url(#v15PulseGrad)')
    ring.setAttribute('filter', 'url(#v15PulseBlur)')
  ...
  function render() {
    ...
    const r = GLOW_BASE_R + (GLOW_MAX_R - GLOW_BASE_R) * glowAmount
    const a0 = GLOW_BASE_A + (GLOW_MAX_A - GLOW_BASE_A) * glowAmount
    const a1 = a0 * 0.35
