...
export function initTetrahedron(): () => void {
  ...
  // without help. Rebind to non-null-typed locals after the guard so the
  // closures inherit the correct type.
  const _hero = document.querySelector('.v24-hero') as HTMLElement | null
  const _stage = document.getElementById('v24Stage') as HTMLElement | null
  const _edgesG = document.getElementById('v24Edges')
  const _glowEl = document.getElementById('v24Glow')
  const _stop0 = document.getElementById('v24GlowStop0')
  const _stop1 = document.getElementById('v24GlowStop1')
  const cursorAura = document.getElementById('v24CursorAura')
  const plasmaLayer = document.getElementById('v24PlasmaLayer')
  const _pulsesG = document.getElementById('v24Pulses')
  const pulseOverlay = document.getElementById('v24PulseOverlay')
  const spacer = document.querySelector('.v24-scroll-spacer') as HTMLElement | null
  ...
  // One-shot decay: first pulse on lock is full size, then each subsequent
  // pulse is PULSE_DECAY_RATIO of the previous. After PULSE_DECAY_COUNT
  // total pulses, the loop stops — no perpetual heartbeat.
  const PULSE_DECAY_COUNT = 4
  const PULSE_DECAY_RATIO = 0.62
  ...
  function completeInteraction() {
    ...
    targetGlow = 1
    window.scrollTo(0, 0)
    window.dispatchEvent(new CustomEvent('v24:complete'))
  }
  ...
  function spawnPlasmaBlob() {
    ...
    blob.className = 'v24-plasma-blob'
  ...
  function spawnPulse(scale: number = 1) {
    const ring = document.createElementNS(SVG_NS, 'circle')
    ring.setAttribute('cx', '0')
    ring.setAttribute('cy', '0')
    ring.setAttribute('r', '40')
    ring.setAttribute('fill', 'url(#v24PulseGrad)')
    ring.setAttribute('filter', 'url(#v24PulseBlur)')
    ring.setAttribute('opacity', '0')
    pulsesG.appendChild(ring)
    const start = performance.now()
    const startR = 40
    const maxR = startR + (PULSE_MAX_R - startR) * scale
    function tick(now: number) {
      const t = Math.min(1, (now - start) / PULSE_DURATION_MS)
      const eased = 1 - Math.pow(1 - t, 2)
      const r = startR + (maxR - startR) * eased
      const opacity = (t < 0.15 ? (t / 0.15) * 0.95 : 0.95 * (1 - (t - 0.15) / 0.85)) * scale
  ...
  function spawnPagePulse(scale: number = 1) {
    ...
    el.className = 'v24-page-pulse'
    pulseOverlay.appendChild(el)
    const start = performance.now()
    const maxSize = 80 + (PAGE_PULSE_MAX_SIZE - 80) * scale
    function tick(now: number) {
      const t = Math.min(1, (now - start) / PAGE_PULSE_DURATION_MS)
      const eased = 1 - Math.pow(1 - t, 2.5)
      const size = 80 + (maxSize - 80) * eased
      const opacity = (t < 0.1 ? t / 0.1 : Math.max(0, 1 - Math.pow((t - 0.1) / 0.9, 1.4))) * scale
  ...
  function render() {
    ...
    if (lockState === 'free') {
      ...
      if (
        pulseCount > 0 &&
        pulseCount < PULSE_DECAY_COUNT &&
        now - pulseStartTime >= PULSE_INTERVAL_MS
      ) {
        const scale = Math.pow(PULSE_DECAY_RATIO, pulseCount)
        spawnPulse(scale)
        spawnPagePulse(scale)
        pulseCount += 1
        pulseStartTime = now
      }
