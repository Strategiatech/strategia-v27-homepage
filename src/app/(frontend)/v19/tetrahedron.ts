...
export function initTetrahedron(): () => void {
  ...
  // Note: v19 reuses v18-prefixed DOM selectors (kept as-is to preserve runtime behaviour).
  const _hero = document.querySelector('.v18-hero') as HTMLElement | null
  const _stage = document.getElementById('v18Stage') as HTMLElement | null
  const _edgesG = document.getElementById('v18Edges')
  const _glowEl = document.getElementById('v18Glow')
  const _stop0 = document.getElementById('v18GlowStop0')
  const _stop1 = document.getElementById('v18GlowStop1')
  const cursorAura = document.getElementById('v18CursorAura')
  const plasmaLayer = document.getElementById('v18PlasmaLayer')
  const _pulsesG = document.getElementById('v18Pulses')
  const pulseOverlay = document.getElementById('v18PulseOverlay')
  const spacer = document.querySelector('.v18-scroll-spacer') as HTMLElement | null
  ...
  function completeInteraction() {
    ...
    targetGlow = 1
    window.scrollTo(0, 0)
    window.dispatchEvent(new CustomEvent('v18:complete'))
  }
  ...
  function spawnPlasmaBlob() {
    ...
    blob.className = 'v18-plasma-blob'
  ...
  const wireBackRGB = { r: 200, g: 230, b: 225 }
  ...
  function spawnPulse() {
    ...
    ring.setAttribute('fill', 'url(#v18PulseGrad)')
    ring.setAttribute('filter', 'url(#v18PulseBlur)')
  ...
  function render() {
    ...
    if (lockState === 'free') {
      const targetSpeed = 1 + (SPEED_BOOST - 1) * targetGlow
      speedBoost += (targetSpeed - speedBoost) * SPEED_EASE
      angleY += ROT_SPEED_Y * speedBoost
      angleX += ROT_SPEED_X * speedBoost
    ...
    const t = now * 0.001
    const breathe = Math.sin(t * 0.8) * 0.12 + Math.sin(t * 1.9 + 0.7) * 0.08 + Math.sin(t * 3.1 + 2.1) * 0.04
    const flicker = Math.sin(t * 4.3 + 1.2) * 0.08 + Math.sin(t * 7.1 + 0.4) * 0.05 + Math.sin(t * 11.3 + 3.0) * 0.03
    const r = (GLOW_BASE_R + (GLOW_MAX_R - GLOW_BASE_R) * glowAmount) * (1 + breathe)
    const a0 = Math.min(1, (GLOW_BASE_A + (GLOW_MAX_A - GLOW_BASE_A) * glowAmount) * (1 + flicker))
    const a1 = a0 * 0.35
